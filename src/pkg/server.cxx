...
bool ServerClient::HandleConnection(
  ...
  try {
    ...
    auto [msg_data, valid] = crypto_driver->decrypt_and_verify(keys.first, keys.second, raw_data);
    if (!valid) throw std::runtime_error("Decrpyption/Verification not valid in HandleConnection");
...
ServerClient::HandleKeyExchange(std::shared_ptr<NetworkDriver> network_driver,
  ...
  server_pub_val_msg.server_signature = crypto_driver->RSA_sign(this->RSA_signing_key, concat_byteblocks(public_key, user_pub_val_msg.public_value));
  ...
  SecByteBlock shared_key = crypto_driver->DH_generate_shared_key(dh, private_key, user_pub_val_msg.public_value);
...
void ServerClient::HandleLogin(
  ...
  if (user_row.user_id == "") throw std::runtime_error("Invalid user ID");

  // Send user's salt
  ServerToUser_Salt_Message server_salt_msg;
  server_salt_msg.salt = user_row.password_salt;

  std::vector<unsigned char> server_salt_msg_enc = crypto_driver->encrypt_and_tag(keys.first, keys.second, &server_salt_msg);
  ...
  auto [user_hspw_msg_vec, hspw_valid] = crypto_driver->decrypt_and_verify(keys.first, keys.second, user_hspw_msg_enc);

  if (!hspw_valid) throw std::runtime_error("HSPW Decrpyption/Verification not valid in HandleLogin");
  ...
  for (int i = 0; i < std::pow(2, 8*PEPPER_SIZE); i++) {
    ...
    std::string hash_with_peppper = crypto_driver->hash(user_hspw_msg.hspw + byteblock_to_string(integer_to_byteblock(pepper)));
    if (hash_with_peppper == user_row.password_hash) match = true;
  }

  // Throw if user login attempt is incorrect——is this correct behavior?
  if (!match) throw std::runtime_error("Password not valid for login");

  // 2FA
  UserToServer_PRGValue_Message user_2fa_msg;
  std::vector<unsigned char> user_2fa_msg_enc = network_driver->read();
  auto [user_2fa_msg_vec, tfa_valid] = crypto_driver->decrypt_and_verify(keys.first, keys.second, user_2fa_msg_enc);
  if (!tfa_valid) throw std::runtime_error("2FA Decrpyption/Verification not valid in HandleLogin");

  user_2fa_msg.deserialize(user_2fa_msg_vec);
  SecByteBlock time_now = integer_to_byteblock(crypto_driver->nowish());
  SecByteBlock server_prf_val = crypto_driver->prg(string_to_byteblock(user_row.prg_seed), time_now, PRG_SIZE);
  if (byteblock_to_string(user_2fa_msg.value) != byteblock_to_string(server_prf_val)) 
  ...
  auto [user_vk_msg_vec, vk_valid] = crypto_driver->decrypt_and_verify(keys.first, keys.second, user_vk_msg_enc);

  if (!vk_valid) throw std::runtime_error("Verification Key Decrpyption/Verification not valid in HandleLogin");
  ...
  cert_msg.server_signature = crypto_driver->RSA_sign(this->RSA_signing_key, concat_string_and_rsakey(id, user_vk_msg.verification_key));
  ...
  std::vector<unsigned char> server_cert_msg_vec = crypto_driver->encrypt_and_tag(keys.first, keys.second, &server_cert_msg);
...
void ServerClient::HandleRegister(
  ...
  if (!hspw_valid) throw std::runtime_error("HSPW Decrpyption/Verification not valid in HandleRegister");

  user_hspw_msg.deserialize(user_hspw_msg_vec);

  // Generate pepper and store hash of salted password + pepper
  SecByteBlock pepper = crypto_driver->png(PEPPER_SIZE);
  user_row.password_hash = crypto_driver->hash(user_hspw_msg.hspw + byteblock_to_string(pepper));
  ...
  std::vector<unsigned char> server_prgseed_msg_enc = crypto_driver->encrypt_and_tag(keys.first, keys.second, &server_prgseed_msg);
  ...
  if (!tfa_valid) throw std::runtime_error("2FA Decrpyption/Verification not valid in HandleRegister");
  ...
  auto [user_vk_msg_vec, vk_valid] = crypto_driver->decrypt_and_verify(keys.first, keys.second, user_vk_msg_data);
...
void handle_2fa(std::shared_ptr<NetworkDriver> network_driver,
    std::shared_ptr<CryptoDriver> crypto_driver,
    std::pair<CryptoPP::SecByteBlock, CryptoPP::SecByteBlock> keys,
    SecByteBlock prg_seed) {
  ...
  SecByteBlock time_now = integer_to_byteblock(crypto_driver->nowish());
  SecByteBlock server_prf_val = crypto_driver->prg(prg_seed, time_now, time_now.size());

  if (byteblock_to_string(user_2fa_msg.value) != byteblock_to_string(server_prf_val)) 
    throw std::runtime_error("2FA failed during register");
}

void handle_certificate(std::shared_ptr<NetworkDriver> network_driver,
    std::shared_ptr<CryptoDriver> crypto_driver,
    std::pair<CryptoPP::SecByteBlock, CryptoPP::SecByteBlock> keys, std::string id,
  ...
  cert_msg.id = id;
  cert_msg.verification_key = user_vk_msg.verification_key;
  cert_msg.server_signature = crypto_driver->RSA_sign(RSA_signing_key, concat_string_and_rsakey(id, user_vk_msg.verification_key));


  // Finally, send the certificate back to the user
  std::vector<unsigned char> server_cert_msg_enc = crypto_driver->encrypt_and_tag(keys.first, keys.second, &cert_msg);
  network_driver->send(server_cert_msg_enc);
}
